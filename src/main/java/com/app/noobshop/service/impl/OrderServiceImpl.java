...
import com.app.noobshop.aop.annotation.business.RemoveOrderDetailRedisCacheAnnotation;
import com.app.noobshop.aop.annotation.business.RemoveOrderSessionAnnotation;
import com.app.noobshop.common.constant.DataConstant;
import com.app.noobshop.common.constant.MessageConstant;
import com.app.noobshop.common.constant.RegexConstants;
import com.app.noobshop.common.context.BaseContext;
import com.app.noobshop.common.exception.PayException;
import com.app.noobshop.common.generator.SnowflakeIdGenerator;
...
import com.app.noobshop.common.result.ScrollQueryResult;
import com.app.noobshop.common.util.DateUtils;
import com.app.noobshop.common.util.SessionUtils;
...
import com.app.noobshop.job.delay.CancelUnpaidOrderDelayJob;
import com.app.noobshop.mapper.OrderMapper;
...
import com.app.noobshop.pojo.emums.OrderPageEnum;
import com.app.noobshop.pojo.emums.OrderStatusEnum;
import com.app.noobshop.pojo.emums.PayTypeEnum;
...
import com.app.noobshop.pojo.vo.OrderWithItemVO;
import com.app.noobshop.pojo.vo.OrderWithTrackingVO;
import com.app.noobshop.service.AddressService;
import com.app.noobshop.service.OrderItemService;
import com.app.noobshop.service.OrderService;
import com.app.noobshop.statemachine.order.OrderStatusTransitionService;
...
public class OrderServiceImpl extends ServiceImpl<OrderMapper, Order> implements OrderService {
    ...
    public Result confirmOrderReceipt(String orderNo) {
        ...
        Order order = lambdaQuery().select(Order::getId, Order::getStatus, Order::getVersion)
                ...
                .one();
        if (Objects.isNull(order)
                || !orderStatusTransitionService.canTransition(order.getStatus(), OrderStatusChangeEvent.RECEIVE)) {
            return Result.error(MessageConstant.SQL_MESSAGE_SAVE_ERROR);
        }
        String now = DateUtils.formatLocalDateTime(LocalDateTime.now());
    ...
    public Boolean updateOrderStatus(Long orderId , String orderNo, OrderStatusEnum orderStatusEnum) {
       ...
       if (Objects.isNull(orderId) && Objects.isNull(orderNo)){
        ...
        if (Objects.equals(order.getStatus(), orderStatusEnum)) {
            RedisConnector.delete(RedisKeyGenerator.orderKey(order.getOrderNo()));
            return true;
        }
        if (!orderStatusTransitionService.canTransition(order.getStatus(), orderStatusEnum)) {
            return false;
        }
        boolean isSuccess;
        if (Objects.isNull(orderId)) {
